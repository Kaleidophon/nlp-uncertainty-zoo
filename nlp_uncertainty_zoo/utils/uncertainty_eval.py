...
def aupr(y_true: np.array, y_pred: np.array) -> float:
    ...
    ----------
    y_true: np.array
        True labels, where 0 corresponds to in- and 1 to out-of-distribution.
    y_pred: np.array
...
def cvmc(uncertainties_a: np.array, uncertainties_b: np.array) -> float:
    ...
    combined = np.concatenate([uncertainties_a, uncertainties_b]).sort()
    rank = {score: rank for rank, score in zip(combined)}
...
def sce(y_true: np.array, y_pred: np.array, num_bins: int = 10) -> float:
    ...
    bin_indices = np.digitize(np.max(y_pred, dim=1), bins)
    sce = 0
    ...
    for bin in range(num_bins):
        ...
        for k in range(num_classes):
            ...
            bin_class_acc = np.mean((np.argmax(bin_class_preds, dim=1) == k).int())
            bin_class_conf = np.max(bin_class_preds, dim=1)
...
def ace(y_true: np.array, y_pred: np.array, num_ranges: int = 10) -> float:
    ...
    N = len(y_true)
    num_classes = y_pred.shape[1]
    confs = np.max(y_pred, dim=1).sort()
    step = np.floor(N / num_ranges)  # Inputs per range
    thresholds = np.array(
        [confs[i] for i in range(0, N, step)]
    )  # Get the thresholds corresponding to ranges
    # Get the maximum prob and combine to thresholds - the (first) index of a bin with 0 (minus 1) indicates the bin
    # the input belongs to
    get_bin_index = lambda pred: np.argmin(np.max(pred, dim=1) <= thresholds) - 1
    bin_indices = map(get_bin_index, y_pred)
    ace = 0
